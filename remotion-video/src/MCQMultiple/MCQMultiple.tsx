...
import { MCQMultipleProps } from "../types";
...
const HookScreen: React.FC<{ hookText: string; examType: string }> = ({ hookText, examType }) => {
  const { fps } = useVideoConfig();
  const frame = useCurrentFrame();
    ...
    <>
      <AbsoluteFill style={{ background: BG }}>
        <div style={{
          ...
          transform: `rotate(-6deg) scale(${interpolate(titleEntrance, [0, 1], [0.3, 1])})`,
        ...
        <div style={{
          ...
          <div style={{
            ...
            <span style={{ fontFamily: comicFont, fontSize: 48, color: WHITE }}>{examType || "PTE"}</span>
          </div>
        </div>
...
/* ═══════════════════════════════════════════════════════════════
   MAIN QUIZ CONTENT — multiple correct answers
   ═══════════════════════════════════════════════════════════════ */
...
const QuizContent: React.FC<{
  ...
  examType: string;
}> = ({ question, options, correctIndices, audioDurationSeconds, audioFileName, examType }) => {
  const { fps } = useVideoConfig();
  const frame = useCurrentFrame();

  const questionBarStart = Math.ceil(0.5 * fps);
  const optionsStart = Math.ceil((audioDurationSeconds + 1.5) * fps);
  const OPTION_STAGGER = Math.ceil(0.3 * fps); // slightly tighter for 5 options
  ...
  // Smaller font/padding for 5 options to fit
  const optFontSize = options.length > 4 ? 38 : 46;
  const optPadding = options.length > 4 ? "28px 40px" : "38px 48px";
  const optGap = options.length > 4 ? 20 : 28;
    ...
    <AbsoluteFill style={{ background: BG }}>
      ...
      <div style={{ position: "absolute", left: 460, top: 290, opacity: titleOpacity }}>
        <div style={{
          ...
          <span style={{ fontFamily: comicFont, fontSize: 44, color: WHITE }}>{examType || "PTE"}</span>
        </div>
      </div>
      ...
      <div style={{ position: "absolute", left: 0, right: 0, top: 540, height: 160, overflow: "hidden" }}>
        <div style={{
          ...
          <p style={{
            fontFamily: sansFont, fontSize: 44, fontWeight: 700,
      ...
      <div style={{
        ...
        display: "flex", flexDirection: "column", gap: optGap,
      }}>
        {options.map((opt, i) => {
          ...
          return (
            <div key={i} style={{ transform: `translateY(${translateY}px)`, opacity }}>
              <div style={{
                ...
                borderRadius: 22, padding: optPadding, boxShadow,
              }}>
                <p style={{
                  fontFamily: sansFont, fontSize: optFontSize, fontWeight: 700,
...
export const MCQMultiple: React.FC<MCQMultipleProps> = ({
  question,
  options,
  correctIndices,
  durationSeconds,
    ...
    <AbsoluteFill>
      {hookIntroDuration > 0 && (
        <Sequence from={0} durationInFrames={hookFrames}>
          <HookScreen hookText="Pick all the right answers!" examType={examType} />
        </Sequence>
      )}
