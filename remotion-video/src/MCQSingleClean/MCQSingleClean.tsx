...
export interface MCQSingleCleanProps {
  headerText: string;
  question: string;
  options: string[];
  correctIndex: number;
...
const MascotSVG: React.FC<{ width?: number }> = ({ width = 340 }) => {
  ...
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 280 250"
      width={width} height={width * (250 / 280)} style={{ display: "block" }}>
      {[0, 1, 2].map((i) => (
        <path key={`wl${i}`} d={`M ${38 - i * 12} ${65} Q ${30 - i * 14} ${87}, ${38 - i * 12} ${110}`}
          fill="none" stroke="#3A5A78" strokeWidth={3} strokeLinecap="round"
          opacity={waveOpacity(i)} transform={`scale(${waveScale(i)})`} style={{ transformOrigin: "52px 87px" }} />
      ))}
      {[0, 1, 2].map((i) => (
        <path key={`wr${i}`} d={`M ${222 + i * 12} ${65} Q ${230 + i * 14} ${87}, ${222 + i * 12} ${110}`}
          fill="none" stroke="#3A5A78" strokeWidth={3} strokeLinecap="round"
          opacity={waveOpacity(i)} transform={`scale(${waveScale(i)})`} style={{ transformOrigin: "208px 87px" }} />
      ))}
...
const HookScreen: React.FC<{ hookText: string; headerText: string }> = ({ hookText, headerText }) => {
  const { fps } = useVideoConfig();
  const frame = useCurrentFrame();
  ...
  // Extract level from headerText, e.g. "IELTS 6 listening" -> "IELTS"
  const badgeText = headerText.includes("IELTS") ? "IELTS" : headerText.split(" ")[0] || "IELTS";
    ...
    <>
      <AbsoluteFill style={{ background: BG }}>
        <div style={{
          ...
          transformOrigin: "left top", opacity: interpolate(titleEntrance, [0, 1], [0, 1]),
        }}>
          <div style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            <span style={{ fontFamily: comicFont, fontSize: 48, color: WHITE }}>{badgeText}</span>
          </div>
        </div>
...
const QuizContent: React.FC<{
  ...
  audioDurationSeconds: number;
  audioFileName: string;
}> = ({ question, options, correctIndex, headerText, audioDurationSeconds, audioFileName }) => {
  const { fps } = useVideoConfig();
  const frame = useCurrentFrame();
    ...
    <AbsoluteFill style={{ background: BG }}>
      ...
      <div style={{ position: "absolute", left: 460, top: 290, opacity: titleOpacity }}>
        <div style={{
          ...
          <span style={{ fontFamily: comicFont, fontSize: 44, color: WHITE }}>{badgeText}</span>
        </div>
      </div>
      ...
      <div style={{ position: "absolute", left: 0, right: 0, top: 540, height: 160, overflow: "hidden" }}>
        <div style={{
          ...
          <p style={{
            fontFamily: sansFont, fontSize: 48, fontWeight: 700,
...
const CTAScreen: React.FC<{ line1: string; line2: string }> = ({ line1, line2 }) => {
  const { fps } = useVideoConfig();
  const frame = useCurrentFrame();

  const entrance = spring({ fps, frame, config: { damping: 16, stiffness: 60, mass: 1 } });
  ...
  return (
    <AbsoluteFill style={{ background: BG }}>
      <div style={{
        ...
        <span style={{
          fontFamily: comicFont, fontSize: 110, color: NAVY,
          letterSpacing: 3, textShadow: "2px 2px 0px rgba(0,0,0,0.1)",
        }}>
          BANDLADDER
        </span>
      </div>
      ...
      <div style={{
        position: "absolute", top: 420, left: 50, right: 50, textAlign: "center",
        opacity: interpolate(entrance, [0, 1], [0, 1]),
      ...
      <div style={{
        position: "absolute", top: 750, left: 0, right: 0,
        display: "flex", flexDirection: "row", justifyContent: "center", alignItems: "center", gap: 40,
      }}>
        {buttons.map((label, i) => {
          const btnEntrance = spring({
            fps, frame: Math.max(0, frame - 10 - i * 6),
          ...
          return (
            <div key={label} style={{
              ...
              <div style={{
                ...
                background: "rgba(255,255,255,0.9)", border: `4px solid ${NAVY}`,
                display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 6,
              }}>
                <span style={{ fontSize: 48 }}>{icons[i]}</span>
                <span style={{ fontFamily: sansFont, fontSize: 26, fontWeight: 700, color: NAVY }}>{label}</span>
              </div>
            </div>
          );
        })}
...
export const MCQSingleClean: React.FC<MCQSingleCleanProps> = ({
  headerText,
  question,
  options,
  correctIndex,
    ...
    <AbsoluteFill>
      {hookDuration > 0 && (
        <Sequence from={0} durationInFrames={hookFrames}>
          <HookScreen hookText={hookText} headerText={headerText} />
        </Sequence>
      )}
      ...
      {ctaDuration > 0 && (
        <Sequence from={ctaStart} durationInFrames={ctaFrames}>
          <CTAScreen line1={ctaLine1} line2={ctaLine2} />
        </Sequence>
      )}
    </AbsoluteFill>
  );
